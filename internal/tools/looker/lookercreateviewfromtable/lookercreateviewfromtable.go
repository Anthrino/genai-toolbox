...
func (cfg Config) Initialize(srcs map[string]sources.Source) (tools.Tool, error) {
	...
	tablesParameter := parameters.NewArrayParameter("tables", `The tables to generate views for.
		Each item can be a 'schema.table' string, or a map with:
...
func (t Tool) Invoke(ctx context.Context, resourceMgr tools.SourceProvider, params parameters.ParamValues, accessToken tools.AccessToken) (any, error) {
	...
	for _, tRaw := range tablesSlice {
		...
		switch t := tRaw.(type) {
		case string:
			// Backward compatibility: handle "schema.table" string
		...
		default:
			return nil, fmt.Errorf("unexpected type in tables list: %T", tRaw)
		}
